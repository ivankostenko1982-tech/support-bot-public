...
# --- newcomer test-only: STEP2 (DM to test user; no imports, no DB) ---
...
@router.chat_member()
async def _newcomer_step2_dm(event):
    try:
    ...
    try:
        log.info("NEWCOMER_TEST STEP2: chat=%s uid=%s %s->%s", chat_id, uid, _st(old), _st(new))
    except Exception:
        pass

    # пробуем отправить DM тест-юзеру; без падений при любых ошибках
    try:
        text = (
            "🧪 <b>Test newcomer DM</b>\n"
            f"Chat: <code>{chat_id}</code>\n"
            f"User: <code>{uid}</code>\n"
            f"Status: <code>{_st(old)} → {_st(new)}</code>"
        )
        # используем глобальный bot, созданный выше в файле
        await bot.send_message(uid, text)
    except Exception as e:
        try:
            log.warning("NEWCOMER_TEST STEP2 DM failed: %r", e)
        except Exception:
            pass
# --- end newcomer test-only STEP2 ---
