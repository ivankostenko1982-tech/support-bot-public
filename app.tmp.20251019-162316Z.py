...
# --- newcomer test-only: STEP1 minimal logger (no DB writes) ---
def setup_newcomer_testonly(router: Router) -> None:
    import os
    def _is_test_pair(chat_id: int, user_id: int) -> bool:
        try:
            tc = int(os.getenv("TEST_CHAT_ID","0") or "0")
            tu = int(os.getenv("TEST_USER_ID","0") or "0")
            tonly = (os.getenv("NEWCOMER_TEST_ONLY","0").lower() in {"1","true","yes","on"})
            return bool(tonly and chat_id and user_id and chat_id==tc and user_id==tu)
        except Exception:
            return False
    @router.chat_member()
    async def _newcomer_step1_log(event: ChatMemberUpdated):
        try:
            ...
            user = getattr(getattr(event, "new_chat_member", None), "user", None) or getattr(event, "from_user", None)
        ...
        try:
            old_status = getattr(getattr(old, "status", None), "value", str(getattr(old, "status", None)))
            new_status = getattr(getattr(new, "status", None), "value", str(getattr(new, "status", None)))
        except Exception:
            old_status = str(getattr(old, "status", None))
            new_status = str(getattr(new, "status", None))
        log.info("NEWCOMER_TEST STEP1: ChatMemberUpdated chat=%s uid=%s %s->%s", chat_id, uid, old_status, new_status)
# ensure one-time configure
setup_newcomer_testonly(router)
# --- end newcomer test-only STEP1 ---
