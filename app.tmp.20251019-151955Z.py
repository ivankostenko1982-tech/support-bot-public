...
# --- newcomer test-only: step1 logger (no DB) ---
...
@router.chat_member()
async def _newcomer_step1_log(event):
    try:
        chat_id = int(getattr(getattr(event, "chat", None), "id", 0) or 0)
        # безопасно достаём user_id
        newcm = getattr(event, "new_chat_member", None)
        uobj = getattr(newcm, "user", None)
        uid = int(getattr(uobj, "id", getattr(getattr(event, "from_user", None), "id", 0)) or 0)
    except Exception:
        chat_id, uid = 0, 0
