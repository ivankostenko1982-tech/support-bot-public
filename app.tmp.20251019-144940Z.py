...
# === BEGIN TEST NEWCOMER (test-only hook) ==================================
# Активируется, когда в ENV установлено:
#   NEWCOMER_TEST_ONLY=1, TEST_CHAT_ID, TEST_USER_ID
try:
    NEWCOMER_TEST_ONLY = (os.getenv("NEWCOMER_TEST_ONLY","0").lower() in {"1","true","yes","on"})
    TEST_CHAT_ID = int(os.getenv("TEST_CHAT_ID","0") or "0")
    TEST_USER_ID = int(os.getenv("TEST_USER_ID","0") or "0")
except Exception:
    NEWCOMER_TEST_ONLY = False
    TEST_CHAT_ID = 0
    TEST_USER_ID = 0

_TEST_NEWCOMER_DONE = False  # простая защита от повторов за один runtime

@router.message(~F.chat.type.in_({ChatType.PRIVATE}))
async def _test_newcomer_min_hook(m: "Message"):
    # В тестовом режиме реагируем только на ПЕРВОЕ сообщение конкретного тест-юзера в тест-чате
    global _TEST_NEWCOMER_DONE
    try:
        if not NEWCOMER_TEST_ONLY or not TEST_CHAT_ID or not TEST_USER_ID:
            return
        if getattr(m.chat, "id", None) != TEST_CHAT_ID:
            return
        u = getattr(m, "from_user", None)
        if not u or getattr(u, "is_bot", False) or getattr(u, "id", None) != TEST_USER_ID:
            return
        if _TEST_NEWCOMER_DONE:
            return

        # 1) Пытаемся удалить первое сообщение тест-новичка
        try:
            await m.delete()
            log.info("TESTNEWCOMER: deleted first message from uid=%s in chat=%s", TEST_USER_ID, TEST_CHAT_ID)
        except Exception as e:
            log.warning("TESTNEWCOMER: delete failed: %r", e)

        # 2) Шлём DM всем человеческим админам чата (ботов фильтруем)
        try:
            admins = await bot.get_chat_administrators(TEST_CHAT_ID)
            text = (
                f"⚠️ Тестовый новичок написал первое сообщение и оно было удалено.\n"
                f"Чат: <code>{TEST_CHAT_ID}</code>\n"
                f"Пользователь: <a href=\"tg://user?id={TEST_USER_ID}\">{TEST_USER_ID}</a>\n"
                f"Проверьте, не бот ли он."
            )
            for a in admins:
                usr = getattr(a, "user", None)
                if not usr or getattr(usr, "is_bot", False):
                    continue
                try:
                    await bot.send_message(usr.id, text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
                except Exception as e:
                    log.debug("TESTNEWCOMER: DM to admin %s failed: %r", getattr(usr, "id", None), e)
        except Exception as e:
            log.warning("TESTNEWCOMER: notify admins failed: %r", e)

        _TEST_NEWCOMER_DONE = True
        log.info("TESTNEWCOMER: hook completed (notified admins)")
    except Exception as e:
        log.exception("TESTNEWCOMER: unexpected error: %r", e)
# === END TEST NEWCOMER ======================================================
