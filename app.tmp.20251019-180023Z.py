...
# --- dynamic hooks loader (safe) ---
try:
    import os, importlib.util
    if os.getenv("ENABLE_HOOKS","0").lower() in {"1","true","yes","on"}:
        hooks_dir = os.getenv("HOOKS_DIR","/opt/tgbots/utils/hooks")
        cand = os.path.join(hooks_dir, "newcomer_hook.py")
        if os.path.isfile(cand):
            spec = importlib.util.spec_from_file_location("newcomer_hook", cand)
            mod = importlib.util.module_from_spec(spec)
            assert spec.loader is not None
            spec.loader.exec_module(mod)  # type: ignore
            if hasattr(mod, "register"):
                mod.register(router, os.environ, log)
                try:
                    log.info("HOOK newcomer loaded from %s", cand)
                except Exception:
                    pass
except Exception as e:
    try:
        log.exception("HOOK load failed: %r", e)
    except Exception:
        pass
# --- end dynamic hooks loader ---
