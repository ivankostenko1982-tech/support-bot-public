...
# --- newcomer sidecar hook (test-only) ---
try:
    from _newcomer_testonly import setup_newcomer_testonly
    setup_newcomer_testonly(router, log if 'log' in globals() else None)
except Exception as _e:
    # не валим бот; просто логируем
    try:
        (log if 'log' in globals() else None).exception("NEWCOMER_TEST load failed: %r", _e)
    except Exception:
        pass
# --- end newcomer sidecar hook ---
