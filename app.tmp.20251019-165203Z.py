...
# --- NEWCOMER_TEST step1: handler without extra imports ---
def _nc_is_test_pair(chat_id, user_id):
    import os
    try:
        tc = int(os.getenv("TEST_CHAT_ID","0") or "0")
        tu = int(os.getenv("TEST_USER_ID","0") or "0")
        tonly = str(os.getenv("NEWCOMER_TEST_ONLY","0") or "0").lower() in {"1","true","yes","on"}
        return bool(tonly and chat_id == tc and user_id == tu)
    except Exception:
        return False

@router.chat_member()
async def _nc_step1(event):
    try:
        chat_id = int(getattr(getattr(event,'chat',None),'id',0) or 0)
        uid = int(getattr(getattr(getattr(event,'new_chat_member',None),'user',None),'id',0) or 0)
        old_status = str(getattr(getattr(event,'old_chat_member',None),'status',None))
        new_status = str(getattr(getattr(event,'new_chat_member',None),'status',None))
    except Exception as ex:
        try:
            log  # ensure logger exists
            log.exception("NEWCOMER_TEST STEP1: parse failed: %r", ex)
        except Exception:
            pass
        return

    if not _nc_is_test_pair(chat_id, uid):
        return

    try:
        log.info("NEWCOMER_TEST STEP1: CMU chat=%s uid=%s %s->%s",
                 chat_id, uid, old_status, new_status)
    except Exception:
        pass
# --- end NEWCOMER_TEST step1 ---
