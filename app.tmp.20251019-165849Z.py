...
# --- newcomer test-only: STEP1 (no imports, no DB) ---
...
async def _newcomer_step1_log(event):
    try:
        ...
        # пробуем разные формы, чтобы не падать
        uid = 0
    ...
    log.info("NEWCOMER_TEST STEP1: chat=%s uid=%s %s->%s", chat_id, uid, _st(old), _st(new))
