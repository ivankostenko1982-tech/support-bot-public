...
# ── TEST NEWCOMER: step1 logger (non-intrusive) ─────────────────────────────
# Включается только при NEWCOMER_TEST_ONLY=1 и при заданных TEST_CHAT_ID/TEST_USER_ID.
# Лишь пишет в журнал факт сообщения от тестового пользователя в тестовом чате.
@router.message()
async def __test_newcomer_step1_logger(m: Message):
    try:
        if os.getenv("NEWCOMER_TEST_ONLY","0").lower() not in {"1","true","yes","on"}:
            return
        if not (TEST_CHAT_ID and TEST_USER_ID):
            return
        chat_id = getattr(getattr(m, "chat", None), "id", None)
        uid = getattr(getattr(m, "from_user", None), "id", None)
        if chat_id != TEST_CHAT_ID or uid != TEST_USER_ID:
            return
        txt = (getattr(m, "text", None) or getattr(m, "caption", None) or "")[:400]
        log.info("TESTNEW step1: msg from test user: chat=%s uid=%s mid=%s text=%r",
                 chat_id, uid, getattr(m, "message_id", None), txt)
    except Exception:
        log.exception("TESTNEW step1 logger crashed")
# ────────────────────────────────────────────────────────────────────────────
async def main():
    init_db()
    # load toggles
